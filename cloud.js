...
function assertFreeLimit(name, limit, label) { const used = getUsage(name); if (used >= limit) throw new Error(`${label}今日已达到免费保护上限 ${limit} 次。明天再用，避免超出免费额度。`); }
...
async function syncCloudLearning() {
  ...
  if (data.records?.length) { state.learning = data.records; localStorage.setItem("paperTrader.learning", JSON.stringify(state.learning)); renderLeaderboard(); }
}
...
async function lineFollowRecommendation() {
  ...
  addFeedback("正在生成实盘参考：读取排行榜策略、获取最新行情、计算止损/止盈/OCO...");
